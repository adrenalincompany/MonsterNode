...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all MonsterNode related functionality
...
/*
    ...
    allow endless transaction that would bloat MonsterNode and make it unusable. To
...
void CPrivateSendServer::CheckTimeout(CConnman& connman)
{
    ...
    int nLagTime = fMasterNode ? 0 : 3000; // if we're the client, give the server a few extra seconds before resetting.
...
{
    if(fLiteMode) return; // disable all MonsterNode specific functionality
    ...
    RenameThread("monsternode-ps-server");
