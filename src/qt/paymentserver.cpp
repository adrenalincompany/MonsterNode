...
const QString BITCOIN_IPC_PREFIX("monsternode:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMNTCTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMNTCTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMNTCT = "application/monsternode-payment";
const char* BIP71_MIMETYPE_PAYMNTCTACK = "application/monsternode-paymentack";
const char* BIP71_MIMETYPE_PAYMNTCTREQUEST = "application/monsternode-paymentrequest";
// BIP70 max payment request size in bytes (DoS protection)
const qint64 BIP70_MAX_PAYMNTCTREQUEST_SIZE = 50000;
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the monsternode: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // monsternode: URI
        {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click monsternode: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start monsternode: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling monsternode: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in monsternode: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // monsternode: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid MonsterNode address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom monsternode addresses are not supported
        ...
        // MonsterNode amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setAttribute(QNetworkRequest::User, BIP70_MESSAGE_PAYMNTCTREQUEST);
    netRequest.setUrl(url);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", BIP71_MIMETYPE_PAYMNTCTREQUEST);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    QNetworkRequest netRequest;
    netRequest.setAttribute(QNetworkRequest::User, BIP70_MESSAGE_PAYMNTCTACK);
    netRequest.setUrl(QString::fromStdString(details.payment_url()));
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, BIP71_MIMETYPE_PAYMNTCT);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", BIP71_MIMETYPE_PAYMNTCTACK);
...
void PaymentServer::netRequestFinished(QNetworkReply* reply)
{
    ...
    if (!verifySize(reply->size())) {
        Q_EMIT message(tr("Payment request rejected"),
            tr("Payment request %1 is too large (%2 bytes, allowed %3 bytes).")
                ...
                .arg(reply->size())
                .arg(BIP70_MAX_PAYMNTCTREQUEST_SIZE),
            CClientUIInterface::MSG_ERROR);
        return;
    }
    ...
    if (requestType == BIP70_MESSAGE_PAYMNTCTREQUEST)
    {
        PaymentRequestPlus request;
        SendCoinsRecipient recipient;
    ...
    else if (requestType == BIP70_MESSAGE_PAYMNTCTACK)
    {
...
{
    bool fVerified = (requestSize <= BIP70_MAX_PAYMNTCTREQUEST_SIZE);
    if (!fVerified) {
