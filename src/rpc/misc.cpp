...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total monsternode balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized monsternode balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "monsternode|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "monsternode")
            + HelpExampleRpc("debug", "monsternode,net")
        );
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"monsternodeaddress\"\n"
            "\nReturn information about the given monsternode address.\n"
            "\nArguments:\n"
            "1. \"monsternodeaddress\"     (string, required) The monsternode address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"monsternodeaddress\", (string) The monsternode address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: MonsterNode address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are monsternode addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) monsternode address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"monsternodeaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"monsternodeaddress\"  (string, required) The monsternode address to use for the signature.\n"
