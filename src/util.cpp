...
const char * const BITCOIN_CONF_FILENAME = "monsternode.conf";
const char * const BITCOIN_PID_FILENAME = "monsternoded.pid";
 ...
 * by well-meaning monsternode adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "monsternode" is a composite category enabling all MonsterNode-related debug output
            if(ptrCategory->count(string("monsternode"))) {
...
#else
    const char* pszModule = "monsternode";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MonsterNodeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MonsterNodeCore
    // Mac: ~/Library/Application Support/MonsterNodeCore
    // Unix: ~/.monsternodecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MonsterNodeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MonsterNodeCore";
#else
    // Unix
    return pathRet / ".monsternodecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty monsternode.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override monsternode.conf
